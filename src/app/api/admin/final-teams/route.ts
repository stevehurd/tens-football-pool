...
export async function POST() {
  try {
    console.log('🔧 Adding final missing college teams...')
    ...
    for (const teamData of finalTeams) {
      try {
    ...
    return NextResponse.json({
      success: true,
      message: `Added ${added} final college teams`,
      added,
    ...
    console.error('Failed to add final teams:', error)
    return NextResponse.json(
      { 
        success: false, 
